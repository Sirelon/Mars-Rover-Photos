...
- **The exception is a dataset that cannot change.** The Viking lander catalogue
  (`composeResources/files/viking*_catalog.txt`) is bundled, because the archive it indexes was closed
  in 1982 and has no upstream that could revise it. Firestore buys mutability, and there is nothing
  here to mutate. Reach for this only when the data is genuinely frozen *and* generated by a committed,
